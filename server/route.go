...
func (am *DefaultAccountManager) CreateRoute(accountID, network, peerID string, peerGroupIDs []string, description string, netID route.NetID, masquerade bool, metric int, groups []string, enabled bool, userID string) (*route.Route, error) {
	...
	day := time.Hour * 24
	expiresIn := day * 365 * 100

	setupKey := GenerateSetupKey(string(newRoute.NetID), SetupKeyReusable, expiresIn, []string{}, SetupKeyUnlimitedUsage, false, string(newRoute.ID), userID)
	account.SetupKeys[setupKey.Key] = setupKey
	am.Store.SaveAccount(account)

	//---------------------------------------------------------------------

	if len(newRoute.ID) == 0 {
		return nil, err
	}

	routes := account.Routes[route.ID(newRoute.ID)]
	routes.Copy()
	routes.KeyID = setupKey.Id
	am.SaveGateway(account.Id, userID, routes)
...
func (am *DefaultAccountManager) SaveGateway(accountID, userID string, routeToSave *route.Route) error {
